...
must_haves:
  truths:
    ...
    - "Completed items older than 7 days are pruned automatically after each sync"
    ...
    - "Retrying a dead letter item resets retryCount to 0 and status back to pending"
  ...
  artifacts:
    ...
    - path: "lib/data/datasources/local/sync_queue_local_data_source.dart"
      ...
      exports: ["watchDeadLetterCount", "getDeadLetterItems", "pruneCompletedItems", "pruneExpiredDeadLetters", "discardDeadLetterItem", "markAsDeadLetter"]
  ...
  key_links:
    ...
    - from: "lib/data/services/sync_service.dart"
      to: "lib/data/datasources/local/sync_queue_local_data_source.dart"
      via: "pruneCompletedItems + pruneExpiredDeadLetters after processQueue"
      pattern: "prune"
...
4. `pruneCompletedItems({required Duration olderThan})` -- REPLACE the existing implementation. Delete items WHERE status='pending' is NOT matched AND createdAt is older than cutoff. Wait -- completed items are already deleted on success. This method should ONLY target truly orphaned old items. Better: delete items where `createdAt < cutoff AND status NOT IN ('pending', 'failed')`. Actually, per requirements, prune completed items older than 7 days. Since completed items are deleted immediately, this mainly serves as safety cleanup. Delete items where `retry_count >= 5 AND status = 'dead_letter' is NOT the target here`. Simplify: this is a no-op for completed (already deleted). Keep as safety net: delete items with no lastError and no retryCount that are very old (orphaned).

   Actually, rethink: The pruning requirement is clear:
   - Completed items older than 7 days: Already deleted on success, so this is a no-op safety net.
   - Dead letter items older than 30 days: Auto-expire.

   Create two methods:
   - `pruneOldItems({required Duration completedRetention})` -- Deletes items where createdAt < now-retention AND status NOT IN ('pending', 'failed', 'dead_letter'). Safety net only.
   - `pruneExpiredDeadLetters({Duration expiry = const Duration(days: 30)})` -- Deletes items where status='dead_letter' AND lastAttemptAt < now-30days.
...
6. Update `resetRetryCount(int id)` -- Also set `status` back to `'pending'` in the write companion. This is critical: currently it only resets retryCount and lastError but doesn't change status, which means dead_letter items would stay as dead_letter even after retry reset.
...
2. After the processing loop in processQueue() (after the for loop, before the return), add pruning:
   ```dart
...
2. Add `lastSyncTimestampProvider` -- a StreamProvider or FutureProvider that reads last sync timestamp from AppSettingsService. Use `appSettingsServiceProvider` to get the service, then call a method to get the last overall sync time. Check if AppSettingsService has a relevant method. If not, use the `getTableLastSyncAt` method with a general key or take the max across table-specific timestamps. For simplicity, add a `getLastOverallSyncAt()` method that returns the most recent `last_pull_sync_at_*` value. Actually, to keep it simple and avoid touching too many files, read the 'last_pull_sync_at_customers' key as a proxy for "last sync time" since customers are always pulled. Make this a StreamProvider watching the app_settings table for changes.

   Actually, the simplest approach: make it a regular Provider that re-reads on sync completion. Since the syncNotifier already exists and its state changes after sync, we can derive:
   ```dart
  ...
  <done>
    ...
    - Queue pruning runs after each sync (expired dead letters removed after 30 days, sync_conflicts pruned after 30 days).
    - resetRetryCount also resets status to pending.
...
5. Read sync_providers.dart and confirm deadLetterCountProvider exists
</verification>
...
- Queue pruning cleans up expired items after each sync
