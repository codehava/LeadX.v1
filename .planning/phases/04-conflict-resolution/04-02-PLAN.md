...
- Per-repository `_server_updated_at` occurrence counts (use `grep -c`):
  - `customer_repository_impl.dart`: 2 occurrences (updateCustomer, updateKeyPerson)
  - `pipeline_repository_impl.dart`: 3 occurrences (updatePipeline, updatePipelineStage, closePipeline/other)
  - `activity_repository_impl.dart`: 3 occurrences (completeActivity, rescheduleActivity, other)
  - `hvc_repository_impl.dart`: 1 occurrence (updateHvc)
  - `broker_repository_impl.dart`: 1 occurrence (updateBroker)
  - `cadence_repository_impl.dart`: 12 occurrences (all update operations on meetings/participants)
  - `pipeline_referral_repository_impl.dart`: 5 occurrences (all update/status-change operations)
  - Total across all repositories: 27 occurrences
