...
Future<void> _applyServerDataLocally(
  ...
  switch (entityType) {
    ...
    default:
      // For other entity types, just update sync metadata
      // The full field mapping for less-commonly-conflicted entities
...
- `_applyServerDataLocally` handles customer, pipeline, activity entity types
...
- `watchRecentConflictCount` method exists in `SyncQueueLocalDataSource`
  ...
  <done>SyncService._processItem uses upsert for creates and version guard for updates. Conflicts are detected via 0-row response, resolved via LWW timestamp comparison, logged to sync_conflicts table, and completed successfully. Server-wins conflicts update local DB with server data for customer/pipeline/activity entities.</done>
</task>
