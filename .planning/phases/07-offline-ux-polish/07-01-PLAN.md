...
must_haves:
  truths:
    - "OfflineBanner appears at top of all shell-wrapped screens when device is offline"
...
<task type="auto">
  <name>Task 2: Create batch sync queue status provider, global last sync timestamp, and shared format utility</name>
  <files>
    lib/data/datasources/local/sync_queue_local_data_source.dart
    lib/presentation/providers/sync_providers.dart
    lib/data/services/app_settings_service.dart
