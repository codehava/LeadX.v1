...
must_haves:
  truths:
    ...
    - "All 7 syncable entity list card types have badge support (Customer, Pipeline, Activity, HVC, Broker, Referral, Cadence Meeting). Key Person is excluded because it only appears in detail screens, which do not show per-entity sync status per locked decision."
  artifacts:
...
<objective>
Add sync queue-aware badges to all entity list cards with failed/dead letter tap navigation, and migrate Activity card from raw icon to SyncStatusBadge.
...
Output: All 7 entity list card types show problem-state badges with tap-to-navigate for failed/dead letter items. Key Person cards (only in detail screens) are excluded per locked decision.
</objective>
...
<task type="auto">
  <name>Task 2: Update HVC, Broker, Referral, and MeetingCard cards with sync queue badges</name>
  <files>
    lib/presentation/widgets/hvc/hvc_card.dart
    lib/presentation/widgets/broker/broker_card.dart
    lib/presentation/widgets/referral/referral_card.dart
  ...
  <action>
    ...
    **Key Person cards are OUT OF SCOPE** -- per locked decision, detail screens do NOT show per-entity sync status. Key Person cards only appear inside detail screens (customer_detail_screen.dart, hvc_detail_screen.dart, broker_detail_screen.dart), so they are excluded.
  ...
  <verify>Run `flutter analyze` and confirm no errors across all 4 modified files. Verify each card widget references `syncQueueEntityStatusMapProvider`.</verify>
  <done>
    ...
    - All 7 syncable entity list card types have consistent badge behavior (Key Person excluded per locked decision)
  </done>
</task>
...
2. All 7 entity list card types (Customer, Pipeline, Activity, HVC, Broker, Referral, MeetingCard) reference `syncQueueEntityStatusMapProvider`
...
6. No detail screen files (customer_detail_screen.dart, hvc_detail_screen.dart, broker_detail_screen.dart) are modified
</verification>

<success_criteria>
- All 7 entity list card types show sync status badges for problem states only (Key Person excluded -- only in detail screens per locked decision)
