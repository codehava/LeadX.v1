...
must_haves:
  truths:
    ...
    - "Drift UserScoreAggregates table has branch_rank, branch_rank_change, regional_rank, regional_rank_change columns matching server schema"
  artifacts:
    - path: "lib/data/database/tables/scoring.dart"
      provides: "Four new nullable integer columns on UserScoreAggregates for branch and regional ranking"
      contains: "branchRank"
...
<task type="auto">
  ...
  <action>
**In `lib/data/database/tables/scoring.dart` (Drift table):**

Add four new nullable integer columns to the `UserScoreAggregates` table to match the server schema additions from Plan 01:
```dart
...
Place them after the existing `rankChange` column (line ~111). These mirror the `branch_rank`, `branch_rank_change`, `regional_rank`, `regional_rank_change` columns added to PostgreSQL in Plan 01's migration.

After adding columns, run `dart run build_runner build --delete-conflicting-outputs` to regenerate `app_database.g.dart`.
