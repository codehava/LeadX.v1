...
must_haves:
  truths:
    - "Ranking SQL function computes DENSE_RANK per role for company-wide pool and populates rank/rank_change on user_score_aggregates"
  ...
  artifacts:
    - path: "supabase/migrations/20260223000001_ranking_functions.sql"
      provides: "calculate_rankings, get_filtered_leaderboard, deactivate_expired_measures SQL functions"
      contains: "DENSE_RANK"
...
<task type="auto">
  ...
  <action>
Create a new SQL migration file with three functions:

**1. `calculate_rankings(p_period_id UUID)`**
- Uses `DENSE_RANK() OVER (PARTITION BY u.role ORDER BY usa.total_score DESC)` to compute company-wide rank per role
- Excludes admins (`u.role NOT IN ('ADMIN', 'SUPERADMIN')`) and inactive users (`u.is_active = TRUE`)
- Users with zero scores are included (they appear at bottom of rankings -- per locked decision)
- After computing rank, calculates rank_change by finding previous period of same type:
  ...
  - `rank_change = previous_rank - current_rank` (positive = improved, negative = dropped)
  - First-ever period: `rank_change = NULL` (client already handles NULL as dash)
...
- For rank_change: reads from user_score_aggregates.rank_change (the pre-computed value, not dynamically computed for filter-specific change)
...
1. DENSE_RANK usage (not RANK) for intuitive tie handling (1,2,2,3 not 1,2,2,4)
2. Proper exclusion of ADMIN/SUPERADMIN roles and inactive users
3. rank_change computed as previous_rank - current_rank (positive = improved)
4. NULL handling for first-ever period
5. get_filtered_leaderboard returns dynamically ranked results
6. deactivate_expired_measures checks period end_date < NOW()
  </verify>
  <done>SQL migration file exists with three valid functions that handle all locked decision requirements: role-based ranking, admin exclusion, zero-score inclusion, rank_change persistence</done>
</task>
...
<verification>
1. SQL migration file exists at `supabase/migrations/20260223000001_ranking_functions.sql` with three functions
2. `calculate_rankings` uses DENSE_RANK with proper PARTITION BY role, excludes admins/inactive
3. `get_filtered_leaderboard` returns dynamically ranked results with optional filters
4. `deactivate_expired_measures` deactivates measures whose period has ended
5. Cron Edge Function calls deactivation then ranking after dirty user processing
6. RLS policies documented and verified sufficient
</verification>

<success_criteria>
Server-side ranking infrastructure is complete: SQL functions exist, cron calls them in correct order, and RLS policies support leaderboard access. The `rank` and `rank_change` columns on `user_score_aggregates` will be populated after the next cron run.
</success_criteria>
